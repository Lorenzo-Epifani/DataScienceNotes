...
In the \textbf{KDE} method, the mathematical expedient used to fix the volume $V$ in the previous example is to define this volume by means of a function, e.g: \begin{eqwbg}
\mathcal{K}(\ov{u}) = \begin{cases}
1, \text{\phantom{aa}\textbf{if:}\phantom{aa}} |u_i| \leq \frac{1}{2}\,\,\, \forall i \in \{1...D\}\\
0, \text{\phantom{aa}\textbf{otherwise}\phantom{aa}}
\end{cases}
...
\begin{aligned}
&\ov{k}(u) \geq 0,\,\, \forall\, \ov{u};\\
&\int k(\ov{u})\,d\ov{u}=1
\end{aligned}
\end{eqwbg}
\subsection{K-nearest neighbor (TODO)}
