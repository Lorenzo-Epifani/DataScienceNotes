...
Probability is a branch of \textbf{pure mathematics}. Probability questions can be posed and solved using \textbf{axiomatic reasoning}, and therefore there is \textbf{one} correct answer to any probability question.\\
Statistical questions can be \textbf{converted} to probability questions by the use of probability models. Once we make certain assumptions about the mechanism generating the data, we can answer statistical questions using probability theory.\\ 
...
In data sciences, \textbf{bayesian statistics} plays a key role. 
...
\textbf{Bayesian statistical} methods use Bayes' theorem to compute and update probabilities after obtaining new data.
\section{Random variables}
Discrete and continuous random variables  require  different assumptions:
...
\end{itemize}
\end{customDef}
TODO - Joint and conditional probability
...
As we mentioned in the introduction, \textbf{indicators} such as mean, mode, variance and other moments
have a slightly different definition depending on whether they refer to \textbf{probability} distributions or \textbf{samples}:
...
%\end{customDef}
\subsection{Moments}
The \textbf{moments} of a generic \textbf{function} are quantitative measures related to the shape of the function's graph. The most general definition of moment is the following:
...
It's important to remember that  \textbf{indicators} (mean,variance,covariance etc) only describe some \textbf{specific properties} of distributions or data-sets.\\
The \textbf{Gini coefficient} is an indicator that measures the \textbf{inequality} among values of a frequency distribution (for example, levels of income). A \textbf{Gini coefficient} of zero expresses perfect equality, where all values are the same (for example, where everyone has the same income). Before to see how to find it, it's required to introduce the \textbf{Lorentz curve}:
...
\]
\end{customDef}
A \textbf{concave} curve represent an \textbf{inhomogeneous} distribution of X, since the curve represent the ratio between the \textbf{expected value} computed over a \textbf{partial support} (from $-\infty$ to $t$) and the \textbf{expected value} on the whole support. A \textbf{linear} curve means that the distribution is \textbf{uniform}. The Gini index and the Lorentz curve are both used to determine the \textbf{relative degree of inequality} of a distribution. They're commonly used to represent the relative income inequality or wealth inequality within a nation or any other group of people. 
 \begin{figure}[H]
                \centering
...
G=\frac{A}{A+B}
\end{eqwbg}
As we can see, $G\in[0,1]$, where $0$ represent the \textbf{uniform} distribution case.
...
\end{customDef}
 \noindent For the (2.22) \textbf{independent} variables have covariance $=0$, thus they re \textbf{uncorrelated}:
 \begin{eqwbg}
...
%\subsection{Zipfian}
%\subsection{Pareto}
\noindent In real cases, observable events do not always provide us with sufficient information to build a reliable model. A good example is the \textbf{spurious correlation}: it occurs when two or more random variables or variables are correlated but not causally related, due to either coincidence or the presence of a certain third,\textbf{ hidden, unobserved or unseen factor} referred to as a \textbf{confounding factor} (or "lurking variable").
...
\end{adjustwidth}
 \begin{figure}[H]
                \centering
                        \includegraphics[width=200px]{pics/Spurious.png}
...
\]
\end{customDef}
Relative entropy is also known as the \textbf{Kullback-Leibler divergence}, and it can be seen as the degree of \textbf{dissimilarity} between $p(x)$ and $q(x)$. It has the following core properties:\begin{itemize}
...
\section{Non-parametric density modelling}
...
In the \textbf{KDE} method, the mathematical expedient used to fix the volume $V$ of the previous example is to define this volume through a function, e.g: \begin{eqwbg}
\mathcal{K}(\ov{u}) = \begin{cases}
1, \text{\phantom{aa}\textbf{if:}\phantom{aa}} |u_i| \leq \frac{1}{2}\,\,\, \forall i \in \{1...D\}\\
0, \text{\phantom{aa}\textbf{otherwise}\phantom{aa}}
\end{cases}
\end{eqwbg}
which represents an unit cube centered on the origin. For a generic point $\ov{x}^*$, the quantity:
\begin{eqwbg}
\mathcal{K}\left(\frac{\ov{x}-\ov{x}^*}{h}\right)
\end{eqwbg}
will be $1$ iif $\ov{x}^*$ lies in the hypercube of side $h$ centered in $\ov{x}$, $0$ otherwise. These functions are called \textbf{kernel functions}, and in this context are called \textbf{Parzen-Rosenblatt windows}.
Assume now that $\mathcal{K}(\ov{x}_c-\ov{x})$ is a generic kernel function centered in $\ov{x}_c$. From $\ref{eqn:nonpar}$, we obtain:
\begin{eqwbg}
\label{eqn:KDE}
p(\ov{x})=\frac{1}{NV_\mathcal{K}}\sum_{n=1}^N \mathcal{K}(\ov{x}_c-\ov{x}_n)
\end{eqwbg}
Where $K$ is obtained by applying the kernel function to \textbf{each point}, and the \textbf{volume} $V$ is replaced with the (shift invariant) volume defined by the kernel function \textbf{itself} (that is $V_\mathcal{K}$).
Let us temporarily \textbf{ignore} the term to the left of the \ref{eqn:KDE}. ($p(\ov{x})$)
We can interpret the right one in 2 ways: 
\begin{itemize}
\item A \textbf{scalar value}: the result is the sum of all the \textbf{contributions} of the kernel function centred in $\ov{x}_c$ \textbf{evaluated} at each point $\ov{x}_n,\,n \in N$;
\item \textbf{A function}: the result is the sum of $N$ \textbf{kernel functions} each one centred in $\ov{x}_n$, where $\ov{x}_c$ is intended as the \textbf{variable} of the function (\textbf{symmetry property}). 
\end{itemize}
In the \textbf{KDE} method, the \textbf{second} interpretation is made, so the final $p(\overline{x})$ will be the result of the contribution of $N$ \textbf{kernel functions}.
A commonly used function as a parzen window is the \textbf{Gaussian}, as it smoothes the final resulting function.
The resulting model can be written as follows:
\begin{eqwbg}[KDE Gaussian model]
p(\overline{x})=\frac{1}{N}\sum_{n=1}^N \frac{1}{(2\pi h^2)^{D/2}}\, \mathtt{exp}\left\{-\frac{\nrm{\ov{x}-\ov{x}_n}^2}{2h^2}\right\}
\end{eqwbg}
Where the volume $V$ of the \ref{eqn:nonpar} has become the normalization constant $(2\pi h^2)^{D/2}$ containing the \textbf{standard deviation} ($h$) of the Gaussian. Indeed, it is reasonable that if we would consider the 'volume' of a Gaussian, this would be \textbf{proportional} to its \textbf{variance}.
The parameter describing the "volume" acts as "\textbf{smoothing}" parameter. Any kernel function $\ov{k}(u)$ subject to the following constraints can be chosen:\begin{eqwbg}[Kernel function constraints]
\begin{aligned}
&k(\ov{u}) \geq 0,\,\, \forall\, \ov{u};\\
&\int k(\ov{u})\,d\ov{u}=1
\end{aligned}
\end{eqwbg}
\subsection{K-nearest neighbor}
\textbf{K-nearest neighbor} (\textbf{KNN}) is a \textbf{non-parametric} density estimation method that, with respect to the \ref{eqn:nonpar}, focuses on fixing  $K$ and finding an appropriate value of $V$ from the data.
Taking as an example the case of a distribution of points on a \textbf{two-dimensional} Euclidean space ($\mathcal{D}=2$), suppose we have $N$ samples. Once that we fix a $K=K^*$, we can evaluate a point $\ov{x}$ on the space: the value of $V$ will be the area of the smallest circle that collects the $K^*$ points closest to $\ov{x}$. It is intuitable that in this case $K$ acts as a \textbf{smoothing hyperparameter}. Having thus defined $V$ as function of $\overline{x}$ and $K^*$:
\begin{eqwbg}
V=f_V(K^*,\ov{x})
\end{eqwbg}
The general formula \ref{eqn:nonpar} become:\begin{eqwbg}
p(\ov{x})=\frac{K}{NV} \underset{\text{KNN case}} {\xrightarrow{\hspace*{2cm}}} \frac{K^*}{Nf_V(K^*,\ov{x})}
\end{eqwbg}
\fastpic{pics/KNN.png}{Representation of the KNN method applied to a $\mathcal{D}=2$ dimensional euclidean space. As we can see, the volume $V$ (represented by the area of the red circle) is function of the evaluated point $\ov{x}$ and the smoothing hyperparameter $K$. In this example, as we can see, $K:=8$}{0.7}
\noindent \textbf{KNN} technique can be easily extended to solve a $M$-class \textbf{classification} problem. \\
Assuming $K_i$ and $N_i$ as the number of points (of $K$ and $N$ \textbf{respectively}) belonging to the class $C_i, \,i\in \{1,...,M\}$:
\begin{eqwbg}
\begin{aligned}
&\sum_i K_i=K,\\
&\sum_i N_i=N
\end{aligned}
\end{eqwbg}
For the class $C_i,$ starting from  the \ref{eqn:nonpar} we obtain:
\begin{eqwbg}
\begin{aligned}
&p(\ov{x}|C_i)= \frac{K_i}{N_iV},\\
&p(C_i)=\frac{N_i}{N}
\end{aligned}
\end{eqwbg}
We can now combine this result using Bayes’ theorem to obtain
the \textbf{posterior} probability of \textbf{class membership} $p(C_i|\ov{x})$:
\begin{eqwbg}
p(C_i|\ov{x}) = \frac{p(\ov{x}|C_i)p(C_i)}{p(\ov{x})}= \frac{K_i}{K}
\end{eqwbg}
If we wish to \textbf{minimize} the probability of \textbf{misclassification}, this is done by assigning
the test point $\ov{x}$ to the class having the \textbf{largest posterior probability}, that is the one with the \textbf{largest ratio} $\frac{K_i}{K}$.
...
\begin{eqwbg}[Cross correlation and Cross coviariance]
\begin{aligned}
...
\end{itemize}
\section{Game theory (TODO)}

\section*{Examples and cases of study}
\subsection*{ANOVA: Analysis of Variance}(TODO)
